...
| 🧑‍💻 김강산 |              |
| 👩‍💻 김하은 |              |
| 🧑‍💻 박혜성 | 보안성을 위해 UUID를 사용했지만, 수작업으로 더미데이터를 입력할 때 사람이 직접 생성하거나 기억하기 어려워 번거로웠다. 외래키가 걸린 테이블에 동일한 UUID를 일일이 맞춰야 해 테스트케이스 구성도 비효율적이었다. 또한, Replica 구현 중 Slave 서버를 복제 무결성을 위해 read_only=1로 설정했지만, SUPER 권한이 있는 사용자는 여전히 INSERT나 UPDATE가 가능하는 점을 몰랐기에 계속해서 불필요한 수정을 하려 했던 어려움이 있었다. |
| 🧑‍💻 윤세진 |              |
