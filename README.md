...
### 🧑‍💻 박혜성  
 - 보안이 요구되는 사용자 ID 등 민감한 칼럼에는 UUID를 사용하였으나, 더미데이터를 수작업으로 입력할 때 사람이 직접 생성하거나 기억하기 어려워 번거로움이 컸다. 특히 외래키로 연결된 테이블에 동일한 UUID를 일일이 매칭해야 했기 때문에, 테스트케이스 구성 또한 비효율적이었다.
