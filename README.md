...
### 🧑‍💻 김강산  
 - 해당 데이터베이스는 유저, 맴버, 업체, 관리자 총 4개의 테이블을 중심으로 구성되어 있는데 테이블 및 컬럼을 계속 추가할 떄마다 외래키 참조 설정이 중복되거나 원하는 논리 구조를 벗어나는 경우가 많아 ERD 구축 작업에만 이틀을 투자할 정도로 ERD 설계를 수정할 때 어려움이 있었다.
