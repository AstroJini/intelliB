...
데이터베이스 덤프(DUMP) 파일만으로 결과를 공유해야 하는 환경에서 개발을 진행하다 보니,
MariaDB/MySQL의 버전, 인코딩, 콜레이션 등 환경 차이에 따른 호환성 문제가 예상보다 컸습니다.

이를 해결하기 위해 여러 환경에서의 쿼리 실행 결과를 검증하고,
문자셋(utf8mb4)과 컬레이션 표준화, 버전별 호환성 테스트를 반복 수행했습니다.
그 결과, 어떤 환경에서도 동일하게 작동하는 스키마 구조와 데이터 일관성을 확보할 수 있었습니다.

이 경험을 통해 “환경 설정의 미세한 차이가 데이터 무결성에 직접적인 영향을 준다”는 점을 깊이 체감했습니다.

🔹 서버 로직 없이 DB만으로 시스템 구현

이번 프로젝트는 별도의 서버 로직 없이 모든 처리를 데이터베이스 내부에서 구현해야 했습니다.
따라서 일반적으로 애플리케이션 계층에서 처리하는
조건 분기, 유효성 검사, 트리거, 제약 조건 관리 등을 전부 SQL로 작성해야 했습니다.

이 과정에서 “어디까지가 데이터베이스의 책임인가”라는 근본적인 질문을 끊임없이 던지게 되었습니다.
결국 DB는 데이터 무결성 보장에 집중하고, 복잡한 비즈니스 로직은 뷰(View)나 임시 테이블로 분리하는 방향으로 설계를 정리했습니다.

이를 통해 DB 중심 구조에서도 명확한 역할 분리와 유지보수성을 확보할 수 있었습니다.

🔹 정규화 수준에 대한 실무적 판단

ERD를 설계하면서 가장 신중했던 부분은 정규화 수준을 어디까지 적용할 것인가였습니다.
이론적으로는 3차 정규화 이상을 유지하는 것이 이상적이지만,
실제 데이터 양과 성능, 관리 효율성을 고려했을 때 과도한 정규화는 비효율적이라는 결론에 이르렀습니다.

따라서 필요에 따라 일부 테이블은 부분 비정규화(De-normalization) 하여 조인 비용을 줄이고,
데이터 무결성과 성능 사이의 균형 잡힌 스키마를 구성했습니다.
이 과정에서 “정규화는 절대적인 규칙이 아니라, 목적과 현실의 균형점이다”라는 교훈을 얻었습니다.

🔹 UUID 설계와 테스트 데이터 관리

보안이 요구되는 사용자 식별자 등 민감한 데이터에는 UUID를 적용했습니다.
그러나 테스트 데이터를 직접 생성해야 하는 상황에서,
UUID를 수동으로 작성하고 외래키가 연결된 테이블마다 일일이 맞추는 일이 비효율적이었습니다.

이를 개선하기 위해 UUID 자동 생성 쿼리 및 테스트 데이터 삽입 스크립트를 작성하여
테스트 과정의 생산성을 높였고, 실수 없이 일관된 데이터 구조를 유지할 수 있었습니다.
이 경험을 통해 “보안을 고려한 설계는 편의성과 자동화 전략을 함께 고민해야 한다”는 점을 체득했습니다.

🔹 Replica 환경 구성과 권한 제어

데이터 무결성을 검증하기 위해 Master–Replica 복제 구조를 직접 구축했습니다.
Replica 서버를 read_only=1로 설정해 변경을 제한했지만,
SUPER 권한이 있는 사용자에게는 여전히 쓰기(INSERT/UPDATE)가 가능하다는 점을 간과했습니다.

이를 통해 권한 제어의 중요성을 실감하고,
SUPER 권한을 분리한 전용 복제 계정을 만들어 복제 무결성을 보장했습니다.
이 과정은 데이터베이스 운영 정책을 세밀하게 설계하는 경험으로 이어졌습니다.
